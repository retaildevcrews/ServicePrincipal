...
{
    internal class ServicePrincipalManager : IDisposable
    {
        ...
        public ServicePrincipalManager(ServicePrincipalSettings spSettings)
        {
            ...
            EmailSettings emailSettings = new EmailSettings(spSettings.Config);
        ...
        {
            string servicePrincipalNamePattern = $"{_spSettings.ServicePrincipalPrefix}";
            ...
            servicePrincipalList = FilterOutSpWithNoSequentialNames(servicePrincipalList);

            return servicePrincipalList;
        }

        private List<ServicePrincipal> FilterOutSpWithNoSequentialNames(List<ServicePrincipal> servicePrincipalList)
        {

            int dummyValue= 0;

            List<ServicePrincipal> filteredList = servicePrincipalList.Where(x => int.TryParse(x.DisplayName.Split('-').ToList().Last(), out dummyValue) == true).ToList();
        ...
        {
            string userNamePattern = $"{_spSettings.UserPrefix}";
        ...
        {
            var servicePrincipalList = GraphHelper.GetAllServicePrincipals($"{_spSettings.ServicePrincipalPrefix}").Result;

            var applicationsList = GraphHelper.GetAllApplicationAsync($"{_spSettings.ServicePrincipalPrefix}").Result;
